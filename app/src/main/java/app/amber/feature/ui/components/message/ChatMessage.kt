...
fun ChatMessage(
    ...
    Column(
        ...
        // P8-02: user bubble 也可切换 variant（原分支/编辑后分支）。单 variant 时
        // 不渲染、不占布局；生成中禁用交互，避免切换截断正在流式写入的下游分支。
        // Minor-1: 禁用条件不限于本消息 loading（仅最后一条消息为 true），
        // 全局生成中（timelineLoading）即禁用，杜绝与服务端守卫之间的竞争窗口。
        if (node.showUserVariantSelector()) {
            ChatMessageBranchSelector(
                node = node,
                onUpdate = onUpdate,
                interactionEnabled = !loading && !timelineLoading,
                modifier = Modifier.fillMaxWidth(),
    ...
    if (showActionsSheet) {
        ChatMessageActionsSheet(
            ...
            onSaveToWorkspace = onSaveToWorkspace?.let { callback -> { callback(message) } },
            onDismissRequest = {
