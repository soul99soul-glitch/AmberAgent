...
enum class VisualRoute {
    /** Strong diagram / chart / schematic signal — emit show-widget SVG. Keep tools available for facts. */
    DIAGRAM_WIDGET,
...
object GenerativeUiPlanner {
    ...
    /**
     * Kept as a compatibility query for the generation handler. Visual
     * requests no longer suppress the tool catalog or force a separate
     * no-tools stream; the normal tool-mediated turn remains available.
     */
    fun needsVisibleStreamingFallback(setting: GenerativeUiSetting, messages: List<UIMessage>): Boolean = false

    /**
     * Compatibility query retained for callers and tests. The tool catalog is
     * never cleared solely because a request is visual; the final widget
     * requirement below is what keeps visual output honest.
     */
    fun shouldGenerateDirectWidgetWithoutTools(setting: GenerativeUiSetting, messages: List<UIMessage>): Boolean = false
    ...
    fun widgetRequirement(setting: GenerativeUiSetting, messages: List<UIMessage>): GenerativeUiWidgetRequirement {
        ...
        return when (route) {
            ...
            VisualRoute.DIAGRAM_WIDGET -> {
                // Tools may provide the facts, but a tool-mediated diagram is
                // still incomplete until the final response contains the
                // requested widget.
