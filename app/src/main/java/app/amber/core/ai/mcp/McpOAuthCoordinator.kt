...
internal class McpOAuthCoordinator(
    ...
    fun startAuthorization(config: McpServerConfig, context: Context): Job {
    ...
    /**
     * 按 serverId 串行刷新。获得锁后重新读取配置，
     * 避免并发工具调用重复使用同一个 refresh token。
     */
    suspend fun ensureFreshToken(configInput: McpServerConfig): McpServerConfig {
        ...
        return lock.withLock {
            ...
            val storedRefreshToken = oauth.refreshToken
            if (!oauth.enabled || storedRefreshToken.isNullOrBlank()) return@withLock config
