...
enum ChatTimelineFollowMode: Equatable {
    case idle
    case followingBottom
    case pausedForUser
}

enum ChatTimelineFollowCommand: Equatable {
    case none
    case requestBottom(reason: String, animated: Bool)
    case showBottomButton(Bool)
}

struct ChatTimelineFollowState: Equatable {
    var mode: ChatTimelineFollowMode = .idle
    var programmaticScrollInProgress = false
    var programmaticScrollToken = 0
    var showBottomButton = false

    mutating func beginProgrammaticScroll() -> Int {
    ...
    @discardableResult
    mutating func endProgrammaticScroll(token: Int) -> Bool {
        guard token == programmaticScrollToken else { return false }
        programmaticScrollInProgress = false
        return true
    }

    mutating func conversationReset() {
        mode = .idle
        programmaticScrollInProgress = false
        programmaticScrollToken &+= 1
...
enum ChatTimelineFollowPolicy {
    static func startGeneration(
        ...
        isAtBottom: Bool,
        isNearEnd: Bool,
        state: inout ChatTimelineFollowState
    ) -> ChatTimelineFollowCommand {
        guard autoScrollEnabled else {
            state.mode = .idle
            return .none
        }
        if isAtBottom || isNearEnd {
            state.mode = .followingBottom
            state.showBottomButton = false
            return .requestBottom(reason: "generation-start", animated: false)
        }
        state.mode = .pausedForUser
        state.showBottomButton = true
        return .showBottomButton(true)
    }

    static func streamChunk(
        autoScrollEnabled: Bool,
        userScrollInTimeline: Bool,
        state: ChatTimelineFollowState
    ) -> ChatTimelineFollowCommand {
        guard autoScrollEnabled,
              state.mode == .followingBottom,
              !userScrollInTimeline else { return .none }
        return .requestBottom(reason: "stream-chunk", animated: false)
    }

    static func scrollProgressChanged(
        isScrollInProgress: Bool,
        userScrollInTimeline: Bool,
        userDragInTimeline: Bool,
        activeGeneration: Bool,
        isAtBottom: Bool,
        state: inout ChatTimelineFollowState
    ) -> ChatTimelineFollowCommand {
        if isScrollInProgress {
            let userDriven = userScrollInTimeline &&
                (!state.programmaticScrollInProgress || userDragInTimeline)
            if activeGeneration && userDriven {
                state.mode = .pausedForUser
                state.showBottomButton = true
                return .showBottomButton(true)
            }
        ...
        guard activeGeneration,
              !state.programmaticScrollInProgress,
              state.mode == .pausedForUser else { return .none }
        if isAtBottom {
            state.mode = .followingBottom
            state.showBottomButton = false
            return .showBottomButton(false)
        }
        state.showBottomButton = true
        return .showBottomButton(true)
    }
...
struct ChatTimelineGenerationEndEffectPlan: Equatable {
    let runEndSettleBeforeIdle: Bool
    let enterIdleAfterEndSettle: Bool
}

enum ChatTimelineGenerationEndSettlePolicy {
    static let maxSettleFrames = 16
    static let requiredStableBottomFrames = 2

    static func effectPlan(
        wasActiveGeneration: Bool,
        activeGeneration: Bool,
        autoScrollEnabled: Bool
    ) -> ChatTimelineGenerationEndEffectPlan {
        if !autoScrollEnabled {
            return ChatTimelineGenerationEndEffectPlan(
    ...
    static func canSettleNow(
        mode: ChatTimelineFollowMode,
        userScrollInTimeline: Bool,
        scrollInProgress: Bool
    ) -> Bool {
        mode == .followingBottom && !userScrollInTimeline && !scrollInProgress
    }

    static func canAttemptSettle(
        mode: ChatTimelineFollowMode,
        userScrollInTimeline: Bool
    ) -> Bool {
        mode == .followingBottom && !userScrollInTimeline
    }

    static func isCloseEnoughToBottom(distance: CGFloat?, bottomBuffer: CGFloat) -> Bool {
        guard let distance else { return false }
        return distance <= bottomBuffer
    }

    static func hasEnoughStableBottomFrames(_ stableBottomFrames: Int) -> Bool {
