...
enum IOSCredentialRedactor {
    ...
    private static func redactValue(_ value: Any, inSearchServices: Bool = false) -> Any {
        if let dict = value as? [String: Any] {
            ...
            for (key, val) in dict {
                ...
                if isSensitiveKey(key) {
                    ...
                    out[key] = redactValue(val, inSearchServices: inSearchServices || lowerKey == "searchservices")
                }
            }
            return out
        }
        if let array = value as? [Any] {
            return array.map { redactValue($0, inSearchServices: inSearchServices) }
        }
        return value
    }
