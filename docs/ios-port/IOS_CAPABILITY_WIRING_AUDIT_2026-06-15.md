...
- MiniApp and WebMount child pages need separate verification before any values or actions can be treated as real. Settings home no longer shows precise fake Memory/Council/SubAgent status or capability-complete Sync/Board/MiniApp/WebMount copy; the Memory, Model Council, SubAgent, Sync/Backup, and Board child pages have now been downgraded to draft/not-wired status.
...
- Hardcoded precise numbers/status: Settings home has been reduced to real current scalar/default/local state or explicit not-wired copy for Provider, TTS, Skill/MCP, Account, ConversationStorage, Appearance, Display, Execution, Runtime, Search, Memory, Sync/Backup, Board, Council, SubAgent, MiniApp, and WebMount. Search services, Memory, Model Council, SubAgent, Sync/Backup, and Board have now been downgraded as child pages too; destination pages such as MiniApp and WebMount still require separate audits because several remain prototype-only.
...
1. Done in Slice 9 for home rows: Settings home subtitles/trailing values now read real local settings where available or show explicit not-wired copy for Search, Memory, Sync/Backup, Board, Council, SubAgent, MiniApp, and WebMount. Search, Memory, Model Council, SubAgent, Sync/Backup, and Board destination pages have also been handled in Slice 11, Slice 12, Slice 13, Slice 14, Slice 15, and Slice 16; remaining destination pages still need separate audits/wiring.
...
3. MiniApp and WebMount separate audits and staged wiring.
