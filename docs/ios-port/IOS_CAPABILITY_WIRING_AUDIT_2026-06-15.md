...
> **Gap note**: Slices 52–62 were produced by independent parallel sessions and are not individually recorded in this audit. Known commits in that range: `80099ea5` (SubAgent runtime — real OpenAIKmpProvider runner), `5fe4625e` (Settings backup export/import — CryptoKit AES.GCM + PBKDF2 + pure-Swift ZIP; also incidentally landed BoardView manual-generation UI), `3f9ea039` (Slice 62 — DuckDuckGo Lite HTML parsing hardening with 2 regex fallbacks). Resume per-slice auditing from Slice 63.

### Slice 63 - Board Collection phase 1 KMP scaffolding (commit `7fd94b80`)

- **Fills a P0 gap**: prior `5fe4625e` landed `BoardView.swift` referencing `IosBoardFactory`/`BoardSignal`/`BoardCollectContext`/`IosBoardAgent`, but the KMP files were never `git add`ed — clean checkout failed to compile. This commit adds them.
- commonMain (`feature/board/api`): `BoardInterfaces.kt` (BoardCollectContext, BoardSignal, BoardSignalCollectorInterface, BoardAgentInterface); `TimeAnchorBoardSignalCollector.kt` (pure-Kotlin time-anchor signal generator via `kotlin.time.Clock.System`).
- iosMain: `IosBoardFactory.kt` (`createCollectors`/`createAgent`/`createTimeCollectContext`); `IosBoardAgent.kt` real `OpenAIKmpProvider.generateText()` adapter with honest degradation (empty signals → empty state, missing API key/model → error message; prompt forbids fabricating calendar/chat/usage facts).
- build.gradle.kts: `api(:ai-core)`, `api(:ai-provider-openai)`, `api(kotlinx.datetime)`, optIn `kotlin.uuid.ExperimentalUuidApi`.
- Verification: `./gradlew :feature:board:api:compileKotlinJvm :compileKotlinIosSimulatorArm64` BUILD SUCCESSFUL; `:shared:linkDebugFrameworkIosSimulatorArm64` BUILD SUCCESSFUL; Shared.h 46 Board-symbol hits. Subagent review PASS all 4 dims, 0 P0/P1.
- Out of scope (phase 1): EventKit calendar signals, hot-list, BGTaskScheduler background collection.

### Slice 64 - iOS search execution chain wired into ChatViewModel (commit `1f5d9742`)

- Closes the iOS search tool-call loop: `makeTextGenerationParams` declares `search_web` tool when `Settings.enableWebSearch` → `provider.streamText` → `onChunk` detects tool call (per-tool-call-id dedup via `detectedToolCallIds`) → `onComplete` runs `executeSearchToolCall` → `IOSSearchExecutor` (DuckDuckGo Lite, real HTTPS) → `messagesByFinishingToolCall` fills the matching tool part's output → re-stream. `maxToolResumeCount=1` bounds recursion.
- ai-core/Tool.kt: `createSearchWebToolDeclaration()` (query required + topic/time_range/max_results).
- ChatView.swift: `ChatToolStepModel` gains optional detail line + search_web branch ('Searching' / 'Search done' + query / first result).
- Tests: `IOSSearchExecutorTests.swift` (searchRequest JSON/plain/empty + DuckDuckGo Lite HTML parsing).
- Verification: `:ai-core` JVM+iOS compile SUCCESSFUL; `:shared:linkDebugFramework` SUCCESSFUL; Shared.h has `createSearchWebToolDeclaration`. Subagent review PASS all 4 dims, 0 P0, 2 P1 (one fixed in Slice 66, one deferred as unreachable).

### Slice 65 - iOS MCP connection management via native URLSession (commit `07c126cd`)

- Plan B (iOS native) chosen because `io.modelcontextprotocol:kotlin-sdk` has no iOS variants (verified in plan).
- KMP (`core/ai/api` commonMain): `McpInterfaces.kt` (McpManagerInterface + McpClientInterface); `McpImportParser.kt` + `McpStatus.kt` git-moved (R093/R100, same package `app.amber.core.ai.mcp`) from `:app` so `:app` references (McpManagementTools, SkillDetailVM, SettingMcpPage) still resolve; `McpToolDeclarations.kt`; build.gradle.kts gains `kotlin("test")` for commonTest; `McpInterfacesTest` + `McpImportParserTest` (Fake manager/client, no network).
- iOS Swift: `IOSMcpClient.swift` (JSON-RPC 2.0 over `URLSessionMcpHTTPTransport`, injectable for tests, SSE event-stream fallback); `IOSMcpConfigStore.swift` (`@Observable` UserDefaults-backed editable local servers); `IOSMcpManager.swift` (`syncAll` connects enabled servers + lists tools + publishes per-server status; `callTool` routes to owning server); `McpServersView.swift` (seed config read-only + local editable servers + real connection status + discovered tools; McpImport/McpAdd save via KMP `McpImportParserKt`/`IOSMcpConfigStore`); `AppShell.swift` injects `IOSMcpConfigStore`; `project.yml` adds `GENERATE_INFOPLIST_FILE: YES` for test target.
- iOS tests (Fake transport/client, no network): `IOSMcpClientTests`, `IOSMcpConfigStoreTests`, `IOSMcpManagerTests`.
- Verification: `:core:ai:api` JVM+iOS compile + allTests SUCCESSFUL; `:shared:linkDebugFramework` SUCCESSFUL (Shared.h 25 MCP-symbol hits); `xcodegen generate`; `xcodebuild build` BUILD SUCCEEDED; `xcodebuild build-for-testing` TEST BUILD SUCCEEDED (test execution timed out on simulator cold-boot — compile-level pass is gating evidence). Subagent review PASS all 4 dims, 0 P0, 1 P1 (fixed in Slice 66).
- KMP seed `Settings.mcpServers` remains read-only; local user-edited servers stored separately in UserDefaults (no seed mutation).

### Slice 66 - P1 review fixes for Slice 64/65 (commit `d9ee5917`)

- Slice 64 P1: search resume branch in `ChatViewModel.onComplete` matched any assistant `search_web` part by name regardless of `enableWebSearch`, so a model hallucinating `search_web` with the feature off would still fire one unauthorized DuckDuckGo request. Now gated on `sharedSettings.snapshot.enableWebSearch`.
- Slice 65 P1: `McpAddView` `needsApproval` toggle was interactive but had no effect (`IOSMcpServerConfig`/`IOSMcpStoredServer` carry no such field; `draftServer` never read it). `McpDraftToggleRow` gains a `disabled` flag; the tools/approval row now renders disabled with subtitle explicitly stating the policy is not yet wired.
- Deferred P1 (Slice 64): `messagesByFinishingToolCall` fills only the first pending tool part — parallel multi-tool `search_web` calls would leave the 2nd+ pending. Unreachable today (providers don't emit parallel search_web; `maxToolResumeCount=1` prevents retry). Tracked as follow-up.
- Verification: `xcodebuild build` BUILD SUCCEEDED (iPhone 17 sim, iOS 26.5).
...
| `7fd94b80` | Board Collection phase 1 KMP scaffolding — fills P0 gap left by `5fe4625e` (which landed `BoardView.swift` referencing KMP symbols that were never `git add`ed). commonMain `BoardInterfaces` + `TimeAnchorBoardSignalCollector`; iosMain `IosBoardFactory` + `IosBoardAgent` (real `OpenAIKmpProvider.generateText` with honest degradation) | `./gradlew :feature:board:api:compileKotlinJvm :compileKotlinIosSimulatorArm64` BUILD SUCCESSFUL; `:shared:linkDebugFrameworkIosSimulatorArm64` BUILD SUCCESSFUL; Shared.h 46 Board-symbol hits; subagent review PASS all 4 dims, 0 P0/P1 | none (build-only verification) | phase 2/3/4 only — EventKit calendar, hot-list, BGTaskScheduler not wired |
| `1f5d9742` | iOS search execution chain wired into ChatViewModel — `search_web` tool declared when `enableWebSearch`; stream detects tool call (per-id dedup); `executeSearchToolCall` runs real `IOSSearchExecutor` (DuckDuckGo Lite HTTPS); result attached to same tool part; re-stream bounded by `maxToolResumeCount=1` | `:ai-core` JVM+iOS compile SUCCESSFUL; `:shared:linkDebugFramework` SUCCESSFUL; Shared.h has `createSearchWebToolDeclaration`; subagent review PASS all 4 dims, 0 P0, 2 P1 (1 fixed Slice 66, 1 deferred as unreachable) | none (build-only verification) | parallel multi-tool search_web only fills first pending part (deferred P1, unreachable today); test execution timed out on simulator cold-boot (compile pass is gating) |
| `07c126cd` | iOS MCP connection management (Plan B native URLSession) — `McpInterfaces` commonMain; `McpImportParser`/`McpStatus` git-moved from `:app` (same package, R093/R100); `McpToolDeclarations`; iOS `IOSMcpClient` (real JSON-RPC + SSE fallback), `IOSMcpConfigStore` (UserDefaults), `IOSMcpManager` (syncAll/callTool); `McpServersView` shows real status + discovered tools; `AppShell` injection | `:core:ai:api` JVM+iOS compile + allTests SUCCESSFUL; `:shared:linkDebugFramework` SUCCESSFUL (Shared.h 25 MCP-symbol hits); `xcodegen generate`; `xcodebuild build` + `build-for-testing` SUCCEEDED; subagent review PASS all 4 dims, 0 P0, 1 P1 (fixed Slice 66) | none (build-only verification) | test execution timed out on simulator cold-boot (compile pass is gating); KMP seed `Settings.mcpServers` read-only; `protocolVersion` pinned 2024-11-05; `callTool` text-only content; `notifications/initialized` not sent |
