...
| **M1.5 (substantial)** | ✅ 大部分完成 | `90068114` + `658dc9d6` + `851df172` + `52ffe7ae` + `f99ca0d5` + `122c12a9` + `5d98aba4` | 蓝图 §E 5 模块拆分，本会话完成 6 个独立子模块：<br>1. **ChatModule** (`90068114`) — 6 singles：PendingMessageStore / ChatService / 3 Orchestrators / WebServerManager<br>2. **MemoryModule** (`851df172`) — 10 singles：MemoryEventLogger / MemoryCandidateFilter / MemoryExtractor / 5×MemoryDream / MemoryFrontmatterCodec / MemoryImportExportManager<br>3. **ICloudModule** (`52ffe7ae`) — 4 singles：ICloudDriveCookieProvider / ICloudDriveClient / ICloudDriveManager / ICloudDriveTools<br>4. **WebMountModule** (`f99ca0d5`) — 33 singles：所有 OAuth 基础设施 + 7 个适配器三件套 + WebMountManager + ViewPool + 4 个 Registry + ProfileBridge + WebMountPrimitiveTools<br>5. **AgentRuntimeModule** (`122c12a9`) — 6 singles：SessionAccessGrantStore + 2 SubAgent + 3 ModelCouncil<br>6. **AgentInfraModule** (`5d98aba4`) — 15 singles：AgentTask/Cron/Live/Terminal/SystemAccess 全套支撑设施<br><br>共 74 个 single 移出 AppModule。AppModule.kt 从 691 行 → **238 行（-66% 净减）**。剩 ~20 个 single 涵盖 ConversationContext / Workspace / FeishuOffice / LocalTools(M1.4) / Highlighter / EmojiUtils / Firebase 等杂项域，作为 follow-up。<br>蓝图原列的 ModelCouncilManager 1150 行单文件拆分仍未做。 |
...
| **M1.5 完整（5 子模块）** | 大部分 | 已做 6 个独立子模块（chat / memory / iCloud / webMount / agentRuntime / agentInfra）共 74 single；剩 conversation context / workspace / feishu office / Highlighter / EmojiUtils / Firebase / search / sync 杂项域 (~20 single) 留待 follow-up |
