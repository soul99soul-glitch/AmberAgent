...
- **三 subagent review（2026-08-14）已修**：①阻断——含 "JSON" 的 4 条分类文案在 `stateSyncFailureMessage(_:)` 字符串路径被纯中文判定折叠回通用文案，已在字符串路径加「剧情同步模型」前缀直透；②段间流式字数串值回跳——事务入口 clear + 每段请求前 `set(0)`；③ordering 测试因空 fixture 实为假 pin——改为直接单测 `orderedSections`（stateLast true/false 各一条全序断言）；④banner detail 无 lineLimit 高度抖动——限 2 行尾截断 + percent 最小宽度 34 防按钮移位。
- review 记录项（未改）：领域层直写呈现层单例的分层气味；Android 无「无 ability→OFF」钳制（KMP 修复收益目前仅 iOS）；Responses API 路径同类「OFF 不下发」当前无触发面；Session 侧失败 banner 与 workspace 侧 VStack 模式不一致（旧有观感问题）。
- **二轮 review（2026-08-14）已修**：①阻断——重写的 ordering 测试调用的 `orderedSections` 在 private extension 内，测试编译不过；已把 `orderedSections` 移出为 internal extension 成员（同文件扩展仍可见 private helper，无警告），stub 改用 `NovelInjectionSection` memberwise init；②`pendingState→stateLast` 接线零覆盖——补 plan 级测试 `testManualSyncPendingStatePlanWiresStateLastOrdering`（`.always` 材料 fixture，断言 material < pendingManualState == user-1）；③banner 的 secondaryHint 会被 detail 两行截断吃掉（「正在停止」「完成前暂不能切换项目」等阻塞解释）——hint 拆独立行 lineLimit(1)；④percent minWidth 34→38（「100%」估算 34-37pt 临界）。
...
- Chat 工具：`skills_list` / `use_skill` / `skill_validate` / `skill_import` / `skill_enable` / `skill_disable` / `soul_import`，以及 `mcp_list` / `mcp_test` / `mcp_import_from_skill`（另保留既有 `mcp_call`）。声明在 KMP `Tool.kt`，执行在 iOS Skill/MCP/Soul 服务。
...
- Review 精准修复：skill enable/list/use 统一以目录名为键；单文件 `SKILL.md` import 顺带 sibling `mcp.json`；前台 `maxToolResumeCount` 4→6（与后台一致）。
- Soul / MCP 最小闭环（2026-08-12，审批策略 2026-08-13 修正）：共享默认 Soul 改为平台中立正文，旧 factory snapshot 才迁移；Android 专用工具说明留在 Android prompt builder。iOS 前台与 child assembler 每个请求恰好注入一次 `agentSoulMarkdown`，不进会话历史。`soul_import` 固定读 `/workspace/SOUL.md`，只读 preview、base/candidate 双 CAS、单槽 previous；设置页「灵魂与记忆」可查看/手工编辑并回退。`soul_import` / `skill_import` / `mcp_import_from_skill` / `recipe_import` 默认弹一次确认卡；普通自动批准不能跳过，高风险自动批准前后台都直接 CAS 应用（后台不再只返回 preview）。`ios.mcp.tool_call=Disabled` 时不声明网络型 MCP 工具，运行时仍硬拒绝，任何自动批准不能绕过。`mcp_import_from_skill` 仍是本地 prepare、脱敏预览、digest 复核、ephemeral connect/list/disconnect，全部成功后一次 `addBatch`；名称冲突全包失败。验证：iPhone 17 Pro Simulator 定点 5/5——`skill_import` 普通自动批准仍弹卡、高风险前台跳卡并落地、高风险后台真正 CAS 应用（不是 preview 假成功）、`recipe_import` 无高风险仍弹卡、后台无高风险仍拒绝。2026-08-13 审查修补：`recipe_import` 已进 `makeTextGenerationParams`（重配置 deferred / 轻配置可见）；`applyPreparedMcpImport` 在 ephemeral connect 前查 Disabled，连通后再查一次防中途关掉；Skill 导入卡不再挂 `local`/`skill_import` 芯片；导入卡标题与时间轴 `recipe_import` 标签对齐；灵魂预览改为限高滚动。定点 `testChatDeclaresSkillAndMcpManagementTools` / `testLightConfigKeepsFullDeclarationList` / `testMcpImportApplyDoesNotConnectWhenNetworkDisabled` / persist 回归绿。审查修补：`start()` / `IOSMcpManager.isEnabled` 在 `ios.mcp.tool_call=Disabled` 时不再 `syncAll`；设置页与 agent 共用 `IOSMcpConfigStore.shared`；import 连通测试后再复核 Disabled。灵魂页 chrome/sheet/审批卡按设置页节奏收过边距与命中区。未验证：真机 Soul/MCP canary、真实 MCP server、Android SDK 编译、设置页真机视觉。
...
- Recipe 可控扩展闭环（2026-08-12 极限瘦身后）：保留 `amber.recipe.v1` schema/validator、稳定 hash 与 CAS 发布、单槽 previous 回退、round 边界动态目录、`recipe__*` 前台执行、mutation step 人工批准/恢复、`recipe_import` 与其它 host-publish 导入同一套开关（普通自动批准仍弹卡，高风险自动批准跳卡）和 Recipes 管理入口。`networkRead` effect class 保留，用于区分本地纯读与网络外发。
- 不再把 Recipe 扩展包装成“自主自进化”：已移除证据投影、自动诊断、独立 evaluator/fixture、自治发布 policy/workflow、审批/通知卡、Experience/Playbook、演化 metrics/receipt，以及对应的大型计划文档和重复测试矩阵。后续若要恢复其中任一层，必须先给出真实 provider 失败样本、可证伪验收和独立产品价值。
- 瘦身后门禁（2026-08-12）：受影响 Swift 全量 `swiftc -parse`；XcodeGen 重建派生工程；iPhone 17 Pro Simulator `build-for-testing`；Recipe / durable run / Skill CAS 回退 / Skill 上下文注入四组 **37/37**；共享层 `DurableRunStoreTest`、`:ai-core:jvmTest`、`:feature:tools:api:jvmTest` 全绿；`git diff --check` 通过。generic Simulator 会额外链接 x86_64，而仓库内 `AmberNative.xcframework` 只有 arm64 simulator slice，因此只以项目约定的 arm64 模拟器门禁为准。
- 旧的 169/169、自主晋升和“真实闭环”复核只代表已删除实现的历史结果，不再作为当前能力声明。仍未覆盖：真机、真实 provider 的 Recipe 生成→批准→下一轮调用，以及真实后台过期/进程强杀恢复。
