...
Last updated: 2026-08-12 (field-write/ghostwrite P0–P3；background execution/recovery P0–P5；self-evolution Slice A–C，两轮独立复核 PASS)
...
- 自进化与热重载：背景与分期见 [`SELF_EVOLUTION_AND_HOT_RELOAD_PLAN.md`](SELF_EVOLUTION_AND_HOT_RELOAD_PLAN.md)（已修正为风险分级自治授权：T0/T1 policy engine 自动、T2 人工）。**Phase 0–4 全部落地**，全程「实现子代理 → 独立 checker 对抗复核 → 精准修复」循环，最终整体复核 PASS：
  ...
  - **真实闭环收口 Slice C + 终审修复（2026-08-12）**：Recipe 反馈已接回现有 ledger/curator，无新 outcome DB 或 lineage 表；Experience 只增加 `sourceArtifactId/sourceArtifactVersion` 两个可选兼容字段。终审同时修复三处已证实缺口：Workspace oracle 的 protected 后置条件参数化并复用于 sealed，硬编码已知样本不再 false-green；自动发布通知捕获 exact previous manifest，A→B→A 的 hash ABA 旧卡无法回退新槽；演化审批正文与最多 8 张通知卡改为局部有界滚动，操作按钮留在滚动区外。门禁：generic iOS Simulator `build-for-testing` 通过；5 条必要定点契约通过（Experience 3、sealed 防硬编码 1、ABA 回退 1）。未做 UI snapshot、真机或真实 provider 端到端。**独立复核（2026-08-12，checker 对抗两轮：PASS_WITH_NOTES + PASS，0 必修 0 建议）**：Slice C 八项契约、最小 UI、5 测试映射、3 完成条件全部成立；三项终审修复的诊断与咬合均成立（protected/sealed 参数化后置条件确实防住硬编码样本、ABA 由 store 层 previous-manifest 比对收口且不误伤正常回退、滚动 UI 按钮在滚动区外且 maxHeight 非固定高度）。复核门禁：14 演化套件 169/169 + 定点 19/19 重跑 + 全量 `xcodebuild build`；`git diff --check` 干净；无探针残留。观察项（记录未修）：harmful 无既有 helpful 时只留账本证据、不进 store harmfulCount（契约 3 允许）；负反馈经 evidence 投影可被手动 analyzeAndImprove 拾起（非自动）；后置条件模板仅支持 string `${input.*}`；rollback 路径 workflow/store 双读冗余；inputBar 批准卡+通知满载时偏高（本次 maxHeight 已缓解）。未验证项不变：真机/真实 provider 端到端、UI snapshot、Watch 审批运行时。
