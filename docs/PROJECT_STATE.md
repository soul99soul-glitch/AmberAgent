...
### 2026-08-02 小说 Session 同步取消文案与重试按钮尺寸（已提交）
...
### 2026-08-01 Chat 思考正文逐词淡入与连续滚动（已提交）
...
### 2026-08-01 Chat 最后一个字后终态跳变修复（已提交）
...
### 2026-08-01 Chat 首行流式冷启动卡顿修复（已提交）
...
### 2026-08-01 Chat 滑动跳变与终态滚动所有权闭环（已提交）
...
### 2026-08-01 Chat 思考动画与流式推理解耦（已提交）
...
### 2026-08-01 Chat 顶部活动岛单行化与中文文案（已提交）
...
### 2026-08-01 Chat 停止搜索与终态工具恢复（已提交）
...
### 2026-08-01 Chat 返回、工具批次与 session 终态修复（已提交）
...
### 2026-08-01 Chat 流式跟随误暂停与长会话入场跳变修复（已提交）
...
### 2026-08-01 OpenAI SSE 流式 UTF-8 跨缓冲区乱码修复（已提交）
...
### 2026-07-31 模型议会：上传文件/图片解析后生成议题（已提交）
...
- 验证：iPhone 17 Pro / iOS 26.5 Simulator 最终八组小说创作回归 **284 passed / 0 failed / 0 skipped**，覆盖 reducer、文档 transition、批量润色、会话与项目 ViewModel、展示和生产 wiring；两个最终只读 subagent 分别复核调用链与测试证据，测试证据审查 PASS。`git diff --check` 通过。改动提交为 `15a8e83ba`；尚未执行真机 Home/锁屏时序、手势、键盘、Dynamic Type、override editor 与长任务视觉验证。
...
- 验证：新增并更新批量润色回归，先复现「取消后 run 泄漏」和「失败后继续累积事务」两条红灯，再转绿；`NovelBatchPolishTests`、单章润色、完整性、会话恢复、ViewModel 与 iOS wiring 相关套件在 iPhone 17 Pro / iOS 26.5 Simulator 共 **161 passed / 0 failed / 0 skipped**。`git diff --check` 通过；改动随后纳入 `15a8e83ba`，尚未执行真机批量润色体验验证。
...
- 切片1 渲染死分支：上轮 hardcode block 无条件 + coalesced=true 后，`ChatMarkdownRendererSelection` 的 liyanan/microsoft/fade 三 case + `LiyananStreamingMarkdownContentView` 类型 + `shouldUseExperimentalMarkdownRenderer`/`shouldUseFadeStreamingRenderer` 孤儿函数 + `hasUsedStreamingMarkdownRenderer` 死 latch 全退役；`selection` 从 2 参收 1 参（block/stable 两态）；`ChatPerformanceSupport` 删 microsoft/liyanan 死 seed；wiring 测试同步为 block/stable 两态。保留 `ChatStableStreamingMarkdownView`（被 block 渲染器的块列表视图用，是活的）。
- 切片2 Chat route 判定层整层拔除：上轮 hardcode native timeline 全开后，`shouldUseNativeTimeline` 无条件 return true，其后 eligibility 判定 + flag 读取全部不可达。删 `ChatCollectionMessageList` 72-217 整段（`ChatMessageListRoute` enum + `ChatMessageListRoutePolicy` + `ChatSwiftUIMessageListFeatureFlags` + 两个 native flag enum + 三个 eligibility enum + `NativeChatTimelineRoutePolicy`）+ `NativeTimelineScrollCore` 的 scroll flag enum；`ChatView.messageList` 去 if/else 直接渲染 `NativeChatTimelineView`、删 `messageListRoute`；`PlaceholderViews` 删 microsoft/liyanan 两 key 常量；测试删 6 个退役判定 func（`ChatMessageProjectionTests` 5 + `NativeTimelineScrollCoreTests` 1）+ 改 1 wiring func 为「ChatView 直接渲染 NativeChatTimelineView、route 层退役」新契约。
- 根因复述：复杂度多来自「为可切换/可回退/可 A/B 预留抽象」，hardcode 后预留层退役但代码+测试残留，本轮拔除。滚动 fallback 安全网（SwiftUI pin/measured-growth 的 `!isNativeScrollDriverDesired` 门控路径）经评估**保留**——hardcode 后双写已不存在，该网是 driver 出错时唯一追底，删它=改行为/牺牲健壮性，违背「简单≠脆弱」。
- iSH「三路径收一」的 oneshot 已于 07-26 commit 成 spawn（生产零调用=已死，本轮无需动）；handoff 剪贴板交接跨 KMP `ai-core/Tool.kt` + 审批 union + 15 文件，属中-高风险跨层手术，按 best judgment **单开下一闭环**（本轮未获用户对该高风险操作的明确确认，且切片1+2 尚待 Xcode 编译验证，不叠加）。
- 事实源修正：本切片使 `ChatSwiftUIMessageList` 不再是默认可用路径，已就地修正本文件旧「默认可用路径是 ChatSwiftUIMessageList」陈述（该陈述上轮 hardcode native timeline 后即过时、本轮才补正）；强制 `ChatStreamReplayTests` 等 replay 门禁直驱哪个列表视图、是否仍覆盖默认 native timeline 路径，列为下一切口核对项（本轮未删死类型，测试仍可编译运行，仅语义上不再代表默认路径）。
- 验证：静态核对全绿——被删符号在 app 代码 0 残留、测试仅剩 `IOSSettingsWiringTests` 合法字符串断言；`ChatMessageListRoute` 词边界 0 命中；`ChatView.messageList` 无条件渲染确认；switch 2 case 对 enum 2 case exhaustive。工作区已提交为 `674840edc`；随后以全新 DerivedData、正常 Debug `iosApp` scheme、Team `89QRFX9548` 完成 generic iOS arm64 构建，KMP `linkDebugFrameworkIosArm64` 与 Xcode 主包均成功，`codesign --verify --deep --strict` 通过，主 Debug dylib SHA-256 为 `780407a39ab448a5d5f6598b8893d95ffe25c5b170b2923185879c351be32148`。02:10 已覆盖安装并启动到 iPhone Air `94918570-0680-5B93-8E38-7E6B355D4426`；设备端回读确认 Amber `app.amber.ios` 1.0 (1) 在新容器 `9A07F888-651D-479F-84F1-60BE3B7EF30B`，主进程与 Activity Widget 均在运行。本次只把完整设备构建作为编译/签名/装机证据，未把它冒充自动化测试绿；默认 native timeline replay 门禁仍待下一切口重建与执行。三路对抗性 subagent review（编译残留扫描 / MessageBubble 逻辑闭环 / ChatView+测试边界+行为等价）确认无编译断裂、无逻辑断裂、无测试边界残骸、行为逐状态等价：保留死类型 `ChatSwiftUIMessageList`/UIKit collection/`NativeChatTimelineView` 内部零引用被删符号；onAppear/onChange 四闭包删 streaming latch 未误伤活逻辑、block latch 链未断；ChatView 19 参数与 `NativeChatTimelineView` init 逐一对应、`.id`/`onDismissKeyboard`/`nativeScrollDriverEnabled:true` 均在；删除前 route 在 hardcode 下恒 = nativeTimelineSwiftUI，故无条件渲染逐状态等价。review 另发现删 Liyanan 类型后 `MessageBubbleView.swift:4 import MarkdownView` 变 unused（模块符号全仓 0 引用、`AmberMarkdownView` 为本地类型不依赖它），已本轮删该 import。
- 残余（本轮不动）：5 处测试注释仍提已删 `hasUsedStreamingMarkdownRenderer`，且 `ChatMessageWidthOverflowTests` 的 config a/b 构造（`TransitioningBubbleHost` 靠 onAppear 流式 arm 已删 latch 再翻 isGenerating=false 来「保持 fade renderer」）与 `ChatStreamReplayTests:1291` 的路径判断，其注释描述的 fade/latched-fade renderer 语义在渲染器简化后已不可复现（现流式/刚完成均走 block），注释与行为脱节、测试判别力退化；三路 review 确认非编译/逻辑断裂（测试仍编译可跑，block 渲染亦产出供溢出审计的视图层级），但属测试配置语义漂移，需下一切口读测试作者意图决定重写 config 还是合并 a/b，本轮不擅自改 `TransitioningBubbleHost` 逻辑以免改坏 canary；`streamingBlockMarkdown`/`coalescedTextBlocks` 两 key 常量 + perf 测试 set 已断开旋钮的 no-op 行；`ChatSwiftUIMessageList`/UIKit `ChatCollectionMessageList` 死类型（保留编译，`#if CHAT_PERF_REPLAY` 仍用前者）。下一切口：handoff 单开闭环 + 上述残余清理 + replay 门禁默认路径覆盖核对 + Xcode 编译/真机验证。

### 2026-07-30 KMP 桥接 decode 补 @Throws(review 修复)

- 对全仓 review 后只修了经代码证据确认的真 bug:两个面向 Swift 的 JSON 桥接 `decode` 消费外部持久化数据(后台 handoff payload、settings JSON),却漏标 `@Throws`。Kotlin/Native 不把未声明的异常桥接成 NSError,损坏输入(突然切后台的半截写、跨版本 schema 不兼容、同步恢复带入坏数据)抛出的 `SerializationException` 会让冷启动恢复 SIGABRT,而不是回退默认配置。这与 `ai-core/Provider.kt` 及两个 provider 明确记录的「@Throws 是 Swift suspend 边界硬契约」一致,这两个 bridge 是遗漏。
- 最小修复落在产生错误事实的层:`IosChatBackgroundPayloadJsonBridge.decode` 与 `IosSettingsJsonBridge.decode` 各加 `@Throws(Throwable::class)`;三处 Swift 调用点改为 `try`(`IOSSharedSettingsStore.decodeSettings` 本就标 `throws`、`IOSChatBackgroundGenerationCoordinator.loadHandoff` 本就在 do/catch 内,二者此前因函数 non-throwing 而静默失效;`IOSSyncBackup` 的备份恢复原为裸调用,现把损坏的 settings.json 转为明确的导入失败)。新增 `KotlinNativeBridgeThrowsContractTest`(源码 canary,断言 decode 紧邻行带 `@Throws`),未修复时 2 条转红、修复后转绿。
- 精准识别排除的假阳性(均经调用链验证,未修):①手表 `approveAnyPendingToolFromWatch` 漏 `pendingAskUser`——askUser 走 `.choose/.answer` → `handleAskUser` 专属路径,根本不进此函数;②image run「DB 停 running」——`start()` 用 `cancel()`(无参)依赖 `IOSRunRecovery` 启动期 sweep 把残留 running 标 interrupted,这是 `recordRun` 注释明示的既定设计,与流式 run 同构;③`UIMessage.appendChunk` 丢 usage——iOS 主流路径(Coordinator/council/agent)用 `MessageStreamAccumulator`,遗留方法在 iOS 生产路径无调用点,只影响 Android。
- GrokWeb 流式永久挂起(原记为「需超时值决策、本轮未动」,精读 JS 后判定根因是确定性结构缺失,无需超时,已修):`IOSGrokWebBrowserTransport.stream` 的 continuation 只靠 JS 回调(complete/error/line-isFinished)或 webView 代理 resume,无超时。两条确定性挂起路径——①注入脚本的 `if (!response.ok || !response.body) return;` 在 HTTP 200 但 `response.body == null`(grok.com 返回空流)时静默 return,既不发 complete 也不发 error,continuation 永久挂起;②未实现 `webViewWebContentProcessDidTerminate`,系统因内存压力终止 WKWebView 内容进程时 fetch 不再 settle 也不触发 didFail,continuation 永久挂起。修复(落在产生挂起的层,非超时兜底):JS 把空 body 改为 `post("complete", {})` 显式收口(非 2xx 仍由 native 的 response 分支按 status 收口);新增 `webViewWebContentProcessDidTerminate` 同时 finishLoading/finishStream 两条 continuation(进程终止可发生在加载或流式任一阶段,两个 finish 各有 guard,只有一个真正 resume),报浏览器错误而非 CancellationError(真实中断不应被当取消吞掉)。新增 `IOSGrokWebStreamTerminalContractTests` 源码 canary 锁这两处结构事实,修复前双向转红、修复后转绿(用独立 swift 脚本对真实源文件与构造的旧版验证)。时序核对:`streamContinuation` 注册先于 `evaluateJavaScript`,进程在 setup 前终止时 completionHandler 报错走 `finishStream`,无 setup 窗口泄漏。
- 验证:`shared:jvmTest`(含新 2 条 canary)全绿;`:shared:compileKotlinIosSimulatorArm64` 与 `:shared:linkDebugFrameworkIosSimulatorArm64` 通过;生成的 ObjC header 确认两个 `decode` 现带 `error:(NSError**)` 即已桥接为 Swift `throws`。完整 `iosApp` scheme 编译因 xcodebuild 的 sandbox-exec 在本环境 `Operation not permitted` 无法解析 SPM 包而未完成(环境限制,非代码问题);GrokWeb 两个修改文件 `swiftc -parse` 0 error。GrokWeb 无端到端自动化测试(需真实 grok.com 会话与可控进程崩溃),运行时取证待真机/模拟器实测:①模拟器内存压力下终止 webview 进程观察是否从永久 generating 变为失败收口;②触发 200+空 body 观察是否正常 complete。工作区另有与本轮正交的 feature flag 移除延伸改动,未冲突。
- subagent 双路 review(异常传播闭环 + 调用链断裂)确认主链路闭环:①每条 throw 都到收口(三处 `try?` 回退完整默认/上一有效快照、`IOSSyncBackup` 经 `SyncBackupView` do/catch 弹 `.error` 给用户、`loadHandoff` catch 返回 nil 由 `handle(_:)` 标 interrupted);②返回值 optional 性无强制解包,Kotlin 内部调用与其它模块均不受影响;③但发现**初次核对遗漏的 2 处测试侧裸调用点**——`IOSParityRedLightTests.swift` 的 `test_assistantCustomHeaders_encode_containsNoCredential`(原 801)与 `test_assistantCustomBodies_encode_containsNoCredential`(原 853)各有一处 `IosSettingsJsonBridge.shared.decode`,decode 变 throwing 后会编译失败。已各加 `try`(所在 throws 测试函数内,`decoded` 经 plain `try` 为非 optional,与 `restoreSnapshot(_ settings: Settings)` 非 optional 签名匹配)。穷尽式 grep 后确认 Swift 侧 5 个调用点(生产 3 + 测试 2)全部带 `try`,无更多遗漏。
