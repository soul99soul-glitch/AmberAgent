...
状态：设计提案（未实施）
日期：2026-07-26
范围：会话页顶部活动岛 `ChatActivityIslandView` 为主，Live Activity 真灵动岛（`ActivityWidget`）做语言对齐。
...
| 壳 | 胶囊边缘辉光 | 锥形渐变对流 + 呼吸 | 谁在动：AI（光谱）/ 工具（单 hue）/ 终态（静态语义色） |
| 文 | 标题文字 | 生成微光扫过 | 正在产出 |
...
- compact：左 glyph + 右计时（现状保留）；锁屏/展开态 center 与上表同文案。
