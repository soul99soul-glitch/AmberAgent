...
| B. Markdown.kt renderer 切到 packed AST | ✅ | renderer consumes packed AST behind MdNode interface (TD.Rust.1a)；markdownAst flag default false，待 parity divergence 裁决 + dogfood。**最终 parity 状态（2026-06-12）**：20/32 hard parity，12 documented parser-level divergences（`MarkdownTreeParityTest` `KNOWN_DIVERGENT_SAMPLES`）。所有 renderer-shape bugs FIXED（BUG #1 heading-drop / BUG #2 inline-code-backtick / class A heading-trim / class B backslash-escape / class C list-item inline-run / class F ordered-list start / class G task-list marker text）；3 个生产 JVM 渲染缺陷作为副作用已修复（heading interior spaces 3711352b、backslash escapes dd7da17c、linkify destination corruption 41081caf）。残余 12 项全为 parser-fidelity 差异（D link/footnote/reference 7 项、E soft-break continuation leading-space 2 项、B-tilde single-tilde strikethrough 1 项、B-CJK CJK-flanked strong emphasis 1 项、C-para paragraph inline-run around MathBlock 1 项）；其中多项 native 侧为 CommonMark-correct。flag 仍默认 false；**B-CJK**（`常被称为**"离线优先"**策略` strong emphasis，pulldown-cmark upstream）为 CJK 用户 flip 主阻断项。 |
