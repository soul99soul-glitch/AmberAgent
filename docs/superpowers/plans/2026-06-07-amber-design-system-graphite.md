...
### Addendum 2 — commits 18–19 (corrections under continued review)

- **Legacy themes truly deleted (D3 fully done):** the 9 `ChatTheme` instances (Whisper..Charcoal) + `ChatThemeChoice` enum + the dead `ChatThemeChoiceCard` were still physically present (the app was only *rewired around* them). Now removed — `ChatTheme.kt` 763→180 lines, `LocalChatTheme` default is Graphite, ~250 bare-hex lines gone. **Criterion 2 ("旧 9 套主题已移除") and the bulk of criterion 6 now met at the code level.** Build + unit suite green.
- **Composer morph correction:** the `+`→`×` morph IS implemented (was mis-reported as missing). `ChatInput.kt` animates the `+` icon rotation 0→45° on expand (a `+` at 45° = `×`) and reveals the `[photo·image·file]` capsule via `AnimatedVisibility`; the flat send fills `accent` on draft. Visually faithful to §6.2 (technique differs: AnimatedVisibility vs width-morph — acceptable). So criterion 4's composer + criterion 5's composer *feature* are met; only the automated UI **test** is device-blocked.
- **Remaining bare hex (criterion 6) are all legitimate:** `theme/*` token sources, `WorkspaceStyle.kt` palette source (derives core colors from the Graphite scheme; its blue/green/amber/red are semantic status tones), `SubAgentAvatar`/`icons/` brand colors, `JsonTree`/markdown syntax colors, `ChatInput*` status tones, `ToolResultPreview` Google brand blue. No component hardcodes the *theme* color.

**Status: commits 1–19. Criteria ①②③ met; ④ & ⑥ met (verified, with legitimate-hex justification); ⑤ context-meter test ✅ + composer/menu features present, their automated UI tests device-blocked (no emulator / no Robolectric); ⑦ preview harness + primitives matrix ✅, deeper coverage constrained by Koin-VM / domain-object coupling.** Foundation, all 9 signature components, all core screens, key + long-tail secondary screens, and a buildable Graphite APK (`assembleDebug` ✅, unit suite ✅) are in place. Genuinely remaining = device/Robolectric for UI-interaction tests, preview fakes for VM-coupled screens, + the user's font binaries & visual review.
