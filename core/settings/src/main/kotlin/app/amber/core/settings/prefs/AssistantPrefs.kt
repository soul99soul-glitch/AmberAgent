...
class AssistantPrefs(
    ...
    private fun readFrom(p: Preferences): AssistantPrefsData {
        val refs = redactor.readRefs(p)
        return AssistantPrefsData(
            assistantId = p[PreferencesKeys.SELECT_ASSISTANT]?.let { it.parseUuidOrNull() }
                ?: DEFAULT_ASSISTANT_ID,
            assistants = (p[PreferencesKeys.ASSISTANTS] ?: "[]")
                .decodeJsonOrNull<List<Assistant>>()
                ?.let { redactor.rehydrateAssistants(it, refs) } ?: emptyList(),
