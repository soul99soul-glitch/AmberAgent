...
object PreferencesKeys {
    ...
    // 永不进入 DataStore）。值为 List<SecretReference> 的 JSON。
    // -----------------------------------------------------------------------
    val SECRET_REFS = stringPreferencesKey("secret_refs")
    ...
    // -----------------------------------------------------------------------
    // Capability parity plan (docs/plans/2026-08-13-android-ios-capability-
    // parity-closure-plan.md, Phase 0). One independent switch per capability;
    // all default OFF. Release turns each on per milestone. Disabling a switch
    // must never delete or rewrite user data — only the boolean key is touched.
    // -----------------------------------------------------------------------
    fun capabilityFlag(id: String): Preferences.Key<Boolean> =
        booleanPreferencesKey("capability_$id")

    // -----------------------------------------------------------------------
    // P2-01 capability permissions (parity plan §8 P2-01). JSON values:
    //  - capability_policies: {"filesystem.write":"auto", ...} — per-capability
    //    disabled/ask/auto policy; a missing entry means "no override".
    //  - capability_policy_overrides: sparse assistant/workspace/conversation
    //    overrides; missing values inherit from the less-specific scope.
    //  - approval_history: recent approval audit entries (digests only, no
    //    plaintext args), capped at APPROVAL_HISTORY_MAX_ENTRIES.
    // -----------------------------------------------------------------------
    val CAPABILITY_POLICIES = stringPreferencesKey("capability_policies")
    val CAPABILITY_POLICY_OVERRIDES = stringPreferencesKey("capability_policy_overrides")
    val APPROVAL_HISTORY = stringPreferencesKey("approval_history")

    // -----------------------------------------------------------------------
    // P4-01 declarative recipes (parity plan §10 P4-01): JSON value of the
    // installed recipe registry (manifests + one previous version). Recipes
    // are declarative tool orchestrations and never carry code or secrets.
    // -----------------------------------------------------------------------
    val RECIPE_REGISTRY = stringPreferencesKey("recipe_registry")

    // -----------------------------------------------------------------------
    // P4-03 persistent JS cells (parity plan §10 P4-03): JSON list of cell
    // records — metadata + store content + terminal state only, never an
    // in-flight JS stack. Disabling the js_cell_runtime flag keeps the data
    // (rollback rules §17.2).
    // -----------------------------------------------------------------------
    val JS_CELLS = stringPreferencesKey("js_cells")
}
