...
internal class StreamingCharRevealClock {
     ...
     *
     * [pruneBelowOffset] lets the caller keep stamps for characters that were
     * just promoted into settled text — they are still mid-fade and continue
     * their own curve on the settled side (see applyStreamingCharReveal).
     */
    fun stamp(
        suffixText: String,
        ...
        maxPending: Int,
        pruneBelowOffset: Int = suffixSourceOffset,
    ) {
        ...
        if (suffixSourceOffset > pruneFloor) {
            appearNanos.keys.removeAll { it < minOf(pruneBelowOffset, suffixSourceOffset) }
        }
