...
class WebDavClient(
    ...
    /**
     * P7-01: MOVE —— 上传先写临时名、完成后 publish 到最终名的标准 WebDAV 原语。
     * Destination 必须是完整 URL（RFC 4918 §9.9），服务器据此重命名。
     */
    suspend fun move(
        source: String,
        destination: String,
        ...
        runCatching {
            val sourceUrl = config.buildUrl(source)
            val destinationUrl = config.buildUrl(destination)
            Log.d(TAG, "MOVE: $sourceUrl -> $destinationUrl")

            val response: HttpResponse = httpClient.request(sourceUrl) {
                method = HttpMethod("MOVE")
                basicAuth(config.username, config.password)
                headers {
                    append("Destination", destinationUrl)
                    append("Overwrite", if (overwrite) "T" else "F")
                }
            }

            // 201 Created / 204 No Content 均为成功；412 Precondition Failed = 目标已存在且 Overwrite: F
            if (!response.status.isSuccess() && response.status != HttpStatusCode.PreconditionFailed) {
                val errorBody = response.bodyAsText()
                Log.e(TAG, "move failed: ${response.status} - $errorBody")
                throw WebDavException("Failed to move: ${response.status}", response.status.value, errorBody)
            }
            if (response.status == HttpStatusCode.PreconditionFailed) {
                Log.e(TAG, "move failed: destination already exists and overwrite=false")
                throw WebDavException(
                    "Failed to move: destination already exists",
                    response.status.value,
                    "",
                )
            }

            Log.d(TAG, "move success: $source -> $destination")
            Unit
        }
    }
