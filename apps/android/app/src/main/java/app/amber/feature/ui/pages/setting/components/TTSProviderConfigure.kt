...
private fun FishAudioTTSConfiguration(
    ...
    // Fish Audio 配置：其余字段（temperature/speed/format/topP/chunkLength/normalize/latency）
    // 走默认值，不暴露在表单中
    // API Key
    FormItem(
    ...
    FormItem(
        ...
        OutlinedTextField(
            ...
            modifier = Modifier.fillMaxWidth(),
            placeholder = { Text("s2.1-pro") }
        )
    }

    // Reference ID
    FormItem(
        label = { Text(stringResource(R.string.setting_tts_page_reference_id)) },
        description = { Text(stringResource(R.string.setting_tts_page_reference_id_description)) }
    ) {
        OutlinedTextField(
