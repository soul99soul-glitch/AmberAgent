...
data class MiniAppBridgeResponse(
    ...
    /**
     * P3-03/P3-04: structured failure code delivered to the MiniApp
     * (e.g. "conversation_not_found", "miniapp.send_disabled",
     * "user_denied"). The JS side attaches it to the rejected Error as
     * `error.code`; plain messages stay backward compatible.
     */
    val errorCode: String? = null,
)

/**
 * Bridge failure with a stable machine-readable [code] (P3-03 "结构化错误回给
 * MiniApp"). [message] stays human-readable for the existing `error` field.
 */
class MiniAppBridgeException(val code: String, message: String) : IllegalArgumentException(message)
