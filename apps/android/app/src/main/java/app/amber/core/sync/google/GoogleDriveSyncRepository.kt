...
class GoogleDriveSyncRepository(
    ...
    /**
     * P7-01：删除单个云端快照。复用现有 driveClient.delete 协议
     * （只做接口适配，不重写 Google Drive 协议）。
     */
    suspend fun delete(session: GoogleDriveAuthSession, file: GoogleDriveFile) =
        withContext(Dispatchers.IO) {
            driveClient.delete(session.accessToken, file.id)
        }
