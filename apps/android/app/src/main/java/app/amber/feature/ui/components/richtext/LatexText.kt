...
/**
 * The settled math renderer and the streaming single-Text fallback must agree
 * on which delimiters are presentation syntax.  Keep the normalization in
 * this one place; the streaming path cannot embed [MathInline] in an
 * AnnotatedString, but it can still show the same formula body.
 */
