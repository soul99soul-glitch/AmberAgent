  ...
  window.AmberBridge = Object.freeze({
    _handleNativeResponse: function (response) {
      ...
      if (response.ok) {
        ...
        // P3-03: structured failure codes surface on the rejected Error as
        // `error.code` (e.g. 'conversation_not_found', 'miniapp.send_disabled').
        const err = new Error(response.error || 'MiniApp bridge error');
        err.code = response.errorCode || 'bridge_error';
        entry.reject(err);
      }
    },
