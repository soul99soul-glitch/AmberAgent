...
class McpManagementToolsTest {
    ...
    fun mcpImportRejectsMissingCandidateDigest() {
        buildJsonObject { put("skill_name", "demo") }.mcpCandidateDigest()
    }

    @Test
    fun mcpImportTrimsCandidateDigest() {
        val digest = buildJsonObject { put("candidate_digest", "  abc123  ") }.mcpCandidateDigest()

        assertEquals("abc123", digest)
    }
}
