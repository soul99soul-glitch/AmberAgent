...
class ToolApprovalPolicyLintTest {
    ...
    fun mutatingNamedToolsDeclareExplicitApprovalBoundary() {
        ...
        val violations = blocks
            ...
            .filterNot { it.name in SELF_GATED_MUTATING_TOOLS }
            .filterNot { block ->
        ...
        // Self-gated entries must still exist where their gate is declared, so a
        // rename/removal forces this list to be revisited instead of silently passing.
        SELF_GATED_MUTATING_TOOLS.forEach { (name, _) ->
            assertTrue("$name is exempted as self-gated but no such Tool(...) exists", blocks.any { it.name == name })
        }
    }
    ...
    companion object {
        ...
        private val NON_MUTATING_WRITE_LIKE_EXCEPTIONS = setOf(
            ...
            "juejin_my_posts",
            "run_plan_update",
            // P4-03: js_cell_create only mutates the sandboxed cell's own
            // metadata inside JsCellRuntime (same pattern as run_plan_update);
            // it never touches user data, and ToolRegistry.invocationPolicy
            // declares the whole js_cell_* family read-only + auto-approvable.
            "js_cell_create",
        )

        /**
         * Mutating tools whose approval boundary is enforced inside their own
         * execute() instead of the generic needsApproval pipeline. The generic
         * pipeline would pause the owning loop at WaitingUser before execute()
         * runs, which these runtimes cannot service; each entry must document
         * where the real gate lives.
         */
        private val SELF_GATED_MUTATING_TOOLS = mapOf(
            "novel_workspace_write" to "Novel canon gate: writes to branches/*/chapters and " +
                "branches/*/plot are buffered as author proposals and only reach disk via " +
                "NovelWorkspaceRuntime.approve(); free paths are a fixed safe whitelist; " +
                "host-owned files refuse. See NovelWorkspaceTools.writeTool.",
        )
